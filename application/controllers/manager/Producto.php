...
class Producto extends CI_Controller {
    ...
    public function __construct() {
        ...
        $model = array('m_usuario', 'm_producto', 'm_categoria');
        ...
        $this->items['get_url'] = base_url() . "manager/producto/";
        $this->items['active'] = "producto";
        $this->items['modulo'] = "producto";
    ...
    public function listar(){
        $data['titulo_pagina'] = 'Listar productos';
        $data['pagina'] = 'Listar productos';
        $data = array_merge($data, $this->items);
        /* ------------------------------------------------------------ */
        $data['lista'] = $this->m_producto->mostrar_todo();
        $data['categorias'] = $this->m_categoria->mostrar_activos();
        $data['tabla'] = $this->smarty_tpl->view('manager/producto_tabla', $data, true);
        $data['form'] = $this->smarty_tpl->view('manager/producto_form', $data, true);
        /* ------------------------------------------------------------ */
    ...
    public function editar($id = ""){
        $data['titulo_pagina'] = 'Editar producto';
        $data['pagina'] = 'Editar producto';
        $data = array_merge($data, $this->items);
        /* ------------------------------------------------------------ */
        
        $data['registro'] = $this->m_producto->mostrar(array("t.id" => $id));
        $data['categorias'] = $this->m_categoria->mostrar_activos();
    ...
    public function accion(){
        ...
        $info['nombre'] = $nombre;
        $info['categoria_id'] = $categoria;

        if ($id == '') { // Agregar
            $resultSet = $this->m_producto->insertar($info);
        }else{
            $resultSet = $this->m_producto->actualizar($info, $id);
        }
        //RESPUESTA
        if($resultSet !== FALSE){
    ...
    public function accion_denegar($id = '') {
        ...
        if (count($this->m_producto->mostrar($where)) == 0) {
            echo $this->url_comp->direccionar($this->items['get_url'] . 'listar', TRUE);
            EXIT;
        } else {
            $this->m_producto->ocultar($id);
            echo $this->url_comp->direccionar($this->items['get_url'] . 'listar', TRUE);
        }
    }
