...
class Login extends CI_Controller {
    ...
    public function __construct() {
        ...
        $helper = array('url');
        $model = array('m_usuario', 'm_cliente');
    ...
    public function ingresar() {
        ...
        $user = $this->m_usuario->mostrar(array("usuario" => $usuario, "clave" => md5($password), "nivel_id" => 4, "oculto" => 0));
        if(count($user) <= 0){
            echo $this->alerta->mensaje_error("Datos incorrectos", TRUE);
            EXIT;
        }
        $data = array('u_cliente' =>
            array(
                'sys_id' => $user['id'],
        ...
        echo $this->url_comp->direccionar(base_url() . "cliente/home");
        EXIT;
    }

    public function salir() {
        $this->session_comp->destruir_session('u_cliente');
        echo $this->alerta->location_href(base_url() . 'cliente', TRUE);
        EXIT;
    }
