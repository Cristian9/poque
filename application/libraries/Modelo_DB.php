...
class Modelo_DB {
    ...
    public function mostrar_random($where, $limite = FALSE, $offset = FALSE) {
        $this->get_query();
        $this->get_where($where);
        if ($limite) {
            return $this->CI->db->order_by("rand()")->limit($limite, $offset)->get()->result_array();
        } else {
            return $this->CI->db->order_by("rand()")->get()->result_array();
        }
    }
